...
// More readable
//===================== modificacion a fecha de nacimiento =============================
function getAge(birthday) {
    var ageDifMs = Date.now() - birthday;
    var ageDate = new Date(ageDifMs); // miliseconds from epoch
    return Math.abs(ageDate.getUTCFullYear() - 1970);
    //La unica modificacion que se realizara fue que si el mes era mayor que la fecha actual
    //o si que el mes es el mismo y el dia mayor que la fecha actual, se le restara
    //debido a que no los ha cumplido annios
    if(getDate.getMonth > Date.now().getMonth || (ageDate.getMonth == Date.now().getMonth && ageDate.getDate > Date.now().getDate)){
        return age-1;
    }
